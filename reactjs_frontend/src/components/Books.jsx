...
function Books() {
    ...
    return (
        <div className='px-5'>
            <h1>Book Storage</h1>
            <div className='pt-2'>
                {role === 'ADMIN' && <button className="btn btn-primary" style={{ marginRight: '5px' }} onClick={() => { navigate('/books/0') }}>Add New Book</button>}
            </div>
                        ...
                        {!isLoading && listBooks ? listBooks.map((item, index) => (
                            <tr key={item.id} className='row-hover'>
                                ...
                                {role === 'ADMIN' && <td>
                                    ...
                                    <button className="btn btn-danger" onClick={() => { window.confirm('Are you sure to delete ?') && dispatch(deleteBook(item.id)) }}>Delete</button>
                                </td>}
                            </tr>
